...
RegisterNetEvent('mt-coke:client:ComecarProcesso', function()
    ...
    QBCore.Functions.Progressbar("CokeProcess1", "STARTING", 5000, false, true, {
...
CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CokeField, true) < 100 then
			SpawncokePlants()
			Wait(500)
		else
			Wait(500)
...
function ValidatePlantsCoord(plantCoord)
	if spawnedPlants > 0 then
		...
		if GetDistanceBetweenCoords(plantCoord, Config.CokeField, false) > 200 then
			validate = false
		end
		return validate
	else
